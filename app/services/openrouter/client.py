...
class OpenRouterClient:
    ...
    def chat_completion(
        ...
        **kwargs: Any,
        ...
        if DEBUG_OPENROUTER and debug_logger:
            ...
            safe_headers = {
                k: v if k != "Authorization" else f"Bearer {v[7:15]}..."
    ...
    def chat_completion_stream(
        ...
        try:
            ...
            for chunk in response.iter_content(
                ...
                if chunk:
                    ...
                    if DEBUG_OPENROUTER and debug_logger and chunk_count % 10 == 0:
                        debug_logger.debug(
                            f"📦 Processed {chunk_count} chunks, buffer size: {len(buffer)}"
                        )
                    ...
                    while True:
                        ...
                        if line.startswith("data: "):
                            ...
                            try:
                                ...
                                if DEBUG_OPENROUTER and debug_logger:
                                    if data_obj.get("choices") and data_obj["choices"][
                                        0
                                    ].get("delta"):
                                        delta = data_obj["choices"][0]["delta"]
                                        if delta.get("content"):
                                            debug_logger.debug(
                                                f"📝 Content chunk: '{delta['content']}'"
                                            )
                                        else:
                                            debug_logger.debug(
                                                f"🔄 Delta (no content): {delta}"
                                            )
                                    else:
                                        debug_logger.debug(
                                            f"📊 Stream data: {data_obj}"
                                        )
                                ...
                                if DEBUG_OPENROUTER and debug_logger:
                                    debug_logger.warning(
                                        f"⚠️ Failed to parse JSON: {data}"
                                    )
                                    debug_logger.warning(
                                        f"⚠️ Parse error: {parse_error}"
                                    )
            ...
            if DEBUG_OPENROUTER and debug_logger:
                debug_logger.info(
                    f"📊 Stream processing complete. Total chunks: {chunk_count}"
                )
